...
def Move7WindowLog(increase):
    ...
    for num in range(col):
        if (increase[0, num+1]==0 and increase[0, num] == 0 ):
            ...
            #move7log[0, num] = math.log((increase[0, num + 1] + gama) / ((increase[0, num] + gama)))
    return move7log
...
def Predict2020GDP( GDP_data):
    ...
    # plt.plot(time, regr1.predict(time), color='red', linewidth=3)
    # plt.scatter(time, GDP_16_19_1, color='red')
    # plt.plot(time, regr2.predict(time), color='black', linewidth=3)
    # plt.scatter(time, GDP_16_19_2, color='black')
    # plt.xlabel(" year ")
    # plt.ylabel("GDP everyday")
    # plt.show()
...
def EvaluateCity(city,start_day,co2_path):
    ...
    illdatapath_usa = "D:\\桌面\\NC\\第二阶段\\实验数据\\result-america.xlsx"
    illdatapath_kor = "D:\\桌面\\NC\\第二阶段\\实验数据\\result-korea.xlsx"
    illdatapath_gbk = "D:\\桌面\\NC\\第二阶段\\实验数据\\result-england.xlsx"
    ...
    if city_name == "武汉":
        eva_len = 6
        evaluate_day = 40
        GDP_loss_std = GetGDPLoss(increase_real, "无提前", evaluate_day -2, eva_len=eva_len)
        evaluate_day += 1
    else :
    ...
    increase_real = GetSimulationIllData(illdatapath_kor, 0)
    GDP_loss = GetGDPLoss(increase_real, "韩国平均", evaluate_day, eva_len=eva_len)
    Treat_cost = GetTreatCost(increase_real, "韩国平均")
    print(city + ":\t 韩国平均: 二氧化碳释放量：(单位：gram/m2/day)：",first_season_co2_emission_predict_2020 * (1 + (GDP_loss_std - GDP_loss) / GDP_loss_std),"\tGDP 相对策略不移动损失(单位：%)：", (GDP_loss - GDP_loss_std) / GDP_loss_std * 100, "\t医疗消耗(单位：亿元)：", Treat_cost)

    increase_real = GetSimulationIllData(illdatapath_kor, 1)

    Treat_cost = GetTreatCost(increase_real,  "韩国up")
    print(city+":\t 韩国up: \t医疗消耗(单位：亿元)：", Treat_cost)

    increase_real = GetSimulationIllData(illdatapath_kor, 2)
    Treat_cost = GetTreatCost(increase_real, "韩国down")
    print(city+":\t 韩国down: \t医疗消耗(单位：亿元)：", Treat_cost)

    increase_real = GetSimulationIllData(illdatapath_gbk, 0)
    GDP_loss= GetGDPLoss(increase_real,  "英国平均",evaluate_day,eva_len=eva_len)
    Treat_cost = GetTreatCost(increase_real,  "英国平均")
    print(city+":\t 英国平均: 二氧化碳释放量：(单位：gram/m2/day)：",first_season_co2_emission_predict_2020*(1+(GDP_loss_std-GDP_loss )/GDP_loss_std), "\tGDP 相对策略不移动损失(单位：%)：", (GDP_loss-GDP_loss_std)/GDP_loss_std*100, "\t医疗消耗(单位：亿元)：", Treat_cost)

    increase_real = GetSimulationIllData(illdatapath_gbk, 1)
    Treat_cost = GetTreatCost(increase_real, "英国up")
    print(city+":\t 英国up: \t医疗消耗(单位：亿元)：", Treat_cost)

    increase_real = GetSimulationIllData(illdatapath_gbk, 2)

    Treat_cost = GetTreatCost(increase_real,  "英国down")
    print(city+":\t 英国down: \t医疗消耗(单位：亿元)：", Treat_cost)

    increase_real = GetSimulationIllData(illdatapath_usa, 0)
    GDP_loss = GetGDPLoss(increase_real, "美国平均", evaluate_day, eva_len=eva_len)
    Treat_cost = GetTreatCost(increase_real, "美国平均")
    print(city+":\t 美国平均: 二氧化碳释放量：(单位：gram/m2/day)：",first_season_co2_emission_predict_2020*(1+(GDP_loss_std-GDP_loss )/GDP_loss_std), "\tGDP 相对策略不移动损失(单位：%)：", (GDP_loss-GDP_loss_std)/GDP_loss_std*100, "\t医疗消耗(单位：亿元)：", Treat_cost)


    increase_real = GetSimulationIllData(illdatapath_usa, 1)
    Treat_cost = GetTreatCost(increase_real,  "美国up")
    print(city + ":\t 美国up: \t医疗消耗(单位：亿元)：", Treat_cost)

    increase_real = GetSimulationIllData(illdatapath_usa, 2)
    Treat_cost = GetTreatCost(increase_real, "美国down")
    print(city + ":\t 美国down: \t医疗消耗(单位：亿元)：", Treat_cost)
